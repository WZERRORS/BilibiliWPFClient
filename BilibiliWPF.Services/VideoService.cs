...
namespace BiliWpf.Services
{
    public class VideoService
    {
        ...
        public async Task<List<RegionVideo>> GetSubRegionSortVideoAsync(int rid, string order, int pn = 1)
        {
            ...
            string url = BiliFactory.UrlContact(Api.REGION_DYNAMIC_CHILD_SORT, param, true);
            var data = await BiliClient.ConvertEntityFromWebAsync<List<RegionVideo>>(url);
            return data;
        }
        /// <summary>
        /// 获取分区排行榜数据
        ...
        public async Task<List<WebVideo>> GetRegionRankAsync(int rid)
        {
            ...
            string url = BiliFactory.UrlContact(Api.REGION_TOTAL_RANK, param, true);
            var data = await BiliClient.ConvertEntityFromWebAsync<List<WebVideo>>(url, "data.list");
            return data;
        }
        /// <summary>
        /// 加载默认的分区内容
        ...
        public async Task<VideoDetail> GetVideoDetailAsync(int aid, string fromSign = "", string bvId = "")
        {
            ...
            string url = BiliFactory.UrlContact(Api.VIDEO_DETAIL_INFO, param, true);
            var data = await BiliClient.ConvertEntityFromWebAsync<VideoDetail>(url, needReferer: true);
            return data;
        }
        /// <summary>
        /// 获取视频简易信息
        ...
        */
        ...
        public async Task<bool> LikeVideoAsync(int aid, bool isLike)
        {
            ...
            var result = await BiliClient.PostContentToWebAsync(Api.VIDEO_LIKE, BiliFactory.UrlContact("", param, true));
        ...
        public async Task<List<FavoriteItem>> GetFavoritesAsync(int aid, int uid)
        {
            ...
            param.Add("pn", "1");
            param.Add("type", "2");
            string url = BiliFactory.UrlContact(Api.VIDEO_FAVORITE_LIST, param, true);
        ...
        public async Task<bool> RepostVideoAsync(string content, int videoId, List<RepostLocation> atIds)
        {
            ...
            var response = await BiliClient.Current.PostContentToWebAsync(Api.VIDEO_REPOST, req);
            if (!string.IsNullOrEmpty(response))
            {
                var jobj = JObject.Parse(response);
                return jobj["code"].ToString() == "0";
